...
export async function POST(request: NextRequest, context: RouteContext) {
  try {
    ...
    const paymentId = await storeAdminService.registerPayment(adminRouteRequest.readPathId(orderId), body);

    return NextResponse.json(backendResponse.success({ paymentId }), { status: 201 });
  } catch (error) {
